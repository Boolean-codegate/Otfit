...
def _instruction(garments: list[GarmentSpec], style: str | None, variation_seed: int) -> str:
    ...
    """
    # 구도/줌 불변 — 인물이 작게 나온 사진에서 임의로 줌인·재구성하는 문제 방지
    retouch_lock = (
        "이 작업은 새 사진 '생성'이 아니라 원본 사진 위에서 옷만 바꾸는 '리터칭'이야. "
        "원본의 카메라 구도, 줌 수준(카메라와 인물의 거리), 인물의 화면 속 크기와 위치, "
        "배경 전체를 1픽셀도 바꾸지 말고 그대로 복사해. 인물을 확대하거나 프레임 안에서 "
        "옮기지 마 — 원본에서 인물이 작고 멀리 있으면 결과에서도 똑같이 작고 멀리 있어야 해."
    )
    # 얼굴·몸 불변 제약 — 모자/신발 착용 시 얼굴·몸 비율이 변형되는 문제 방지
    face_lock = (
        "얼굴은 원본을 복제하듯 정확히 유지해. "
        "눈매·코·입·얼굴형·피부·표정·시선은 물론, 얼굴의 크기와 화면 속 위치·비율까지 "
        "원본과 동일해야 해. 모자를 씌우더라도 얼굴을 다시 그리거나 크기를 바꾸지 말고, "
        "머리 위에 모자만 얹어. 몸의 크기·비율(다리 길이, 어깨 폭 포함)도 원본 그대로 — "
        "아이템 때문에 몸을 키우거나 늘리지 마."
    )
    # 레이어링 규칙 — 겹쳐 입기/바지를 신발에 넣는 부자연스러운 결과 방지
    layering = (
        "레이어링 규칙: 지정한 아이템은 같은 부위의 기존 옷을 완전히 '대체'해 — "
        "기존 옷이 새 옷 밖으로 보이면 안 돼 (원피스가 대체되면 심플한 이너로 자연스럽게 처리). "
        "긴 하의는 신발 위를 덮어야 해 — 하의를 신발이나 부츠 안에 넣지 마. "
        "신발을 바꿔도 다리 길이와 서 있는 위치는 그대로."
    )
    if len(garments) == 1 and garments[0].category != "accessory":
        ...
        text = (
            ...
            + retouch_lock + " " + _PHOTO_TRUTH + " " + face_lock
        )
    else:
        lines = [
