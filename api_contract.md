...
### GET /feed  (auth)  `?sort=hot|new|following` — following은 내가 팔로우한 유저의 게시물만 최신순
...
### POST /posts  (auth)
req: `{ "result_id": "r_1"?, "product_id": "p_1"?, "caption": "…", "before_url": "…"?, "after_url": "…"?, "include_before": true? }`
- `result_id`가 있으면 after/product를 결과에서 자동 결정 (본인 결과만 가능, before는 include_before로 동의할 때만 공개 — 서버가 원본 사진 키를 사용)
